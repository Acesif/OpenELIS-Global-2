...
class StorageAssignmentPage {
  ...
  getRoomDropdown() {
    ...
    return cy.get('[data-testid="room-combobox"], [data-testid="room-dropdown"]');
  }

  getDeviceDropdown() {
    // LocationManagementModal uses EnhancedCascadingMode with comboboxes
    return cy.get('[data-testid="device-combobox"], [data-testid="device-dropdown"]');
  }

  getShelfDropdown() {
    // LocationManagementModal uses EnhancedCascadingMode with comboboxes
    return cy.get('[data-testid="shelf-combobox"], [data-testid="shelf-dropdown"]');
  }

  getRackDropdown() {
    // LocationManagementModal uses EnhancedCascadingMode with comboboxes
    return cy.get('[data-testid="rack-combobox"], [data-testid="rack-dropdown"]');
  }
  ...
  selectRoom(roomName) {
    ...
    cy.get('body').then(($body) => {
      ...
      if (menuText.includes(roomName)) {
        ...
        cy.get('[role="listbox"]').contains("Main", { matchCase: false }).click({ force: true });
      } else {
