...
Cypress.Commands.add("setupStorageTests", () => {
  ...
  if (skipFixtures) {
    cy.log("Skipping fixture loading (SKIP_FIXTURES=true) - assuming fixtures exist");
  } else if (forceFixtures) {
    cy.log("Force loading fixtures (FORCE_FIXTURES=true) - reloading even if exist");
...
Cypress.Commands.add("cleanupStorageTests", () => {
  ...
  if (shouldCleanup) {
    ...
    cy.log("Skipping fixture cleanup (CLEANUP_FIXTURES=false) - fixtures preserved for next run");
  }
});
