...
for template in "${TEMPLATES[@]}"; do
  ...
  for ((i = 1; i <= COUNT; i++)); do
    ...
    pushed="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("pushed",""))' <<< "$response")"
    sent_count="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("count",""))' <<< "$response")"
