...
/**
 * Unit tests for ShortCodeValidationService
 * Following TDD: Write tests BEFORE implementation
 * Tests short code format validation, uniqueness checking, and change warnings
 */
...
public class ShortCodeValidationServiceTest {
     ...
     * T254: Short Code Format - Max 10 chars, alphanumeric, hyphen/underscore allowed
     */
    @Test
    public void testShortCodeFormat() {
        // Valid formats
        assertTrue("Valid short code: FRZ01", 
            shortCodeValidationService.validateFormat("FRZ01").isValid());
        assertTrue("Valid short code: A1-B2", 
            shortCodeValidationService.validateFormat("A1-B2").isValid());
        assertTrue("Valid short code: RACK_1", 
            shortCodeValidationService.validateFormat("RACK_1").isValid());
        ...
        assertFalse("Invalid: contains special chars", 
            shortCodeValidationService.validateFormat("FRZ@01").isValid());
        assertFalse("Invalid: contains spaces", 
            shortCodeValidationService.validateFormat("FRZ 01").isValid());
        assertFalse("Invalid: empty string", 
            shortCodeValidationService.validateFormat("").isValid());
        assertFalse("Invalid: null", 
            shortCodeValidationService.validateFormat(null).isValid());
    }

    /**
     * Test auto-uppercase conversion
     * T254: Auto-Uppercase Conversion - Input auto-converted to uppercase
     */
    @Test
     ...
     * T254: Must Start With Letter Or Number - Reject codes starting with hyphen/underscore
     */
    @Test
    public void testMustStartWithLetterOrNumber() {
        // Valid: starts with letter
        assertTrue("Valid: starts with letter", 
            shortCodeValidationService.validateFormat("FRZ01").isValid());
        
        // Valid: starts with number
        assertTrue("Valid: starts with number", 
            shortCodeValidationService.validateFormat("1RACK").isValid());

        // Invalid: starts with hyphen
        assertFalse("Invalid: starts with hyphen", 
            shortCodeValidationService.validateFormat("-FRZ01").isValid());

        // Invalid: starts with underscore
        assertFalse("Invalid: starts with underscore", 
            shortCodeValidationService.validateFormat("_RACK1").isValid());
    }
    ...
    /**
     * Test warning when changing short code
     * T254: Warning When Changing Short Code - Warning generated when short code changes
     */
    @Test
    public void testWarningWhenChangingShortCode() {
        ...
        assertTrue("Warning should mention short code change", 
            warning.contains("FRZ01") || warning.contains("FRZ02"));
    }
}
